...
aws autoscaling create-launch-configuration --launch-configuration-name Project1-launch-config --image-id $1 --key-name $4 --security-groups $5 --instance-type $3 --user-data /home/controller/Documents/MP1_Environment_Setup_Final/install-webserver.sh --iam-instance-profile $7
...
aws cloudwatch put-metric-alarm --alarm-name Cloudwatchalarm --metric-name Cloudwatch --namespace AWS/EC2 --statistic Average maximum=3 --period 60 --threshold 30 --comparison-operator GreaterThanOrEqualToThreshold --dimensions "Name=AutoScalingGroup,Value=Project1-auto-scaling-group-2" --evaluation-periods 1 --alarm-actions arn:aws:autoscaling:us-west-2:089423386606:scalingPolicy:cc88d707-2094-4129-9525-f91079eec50f:autoScalingGroupName/Project1-auto-scaling-group-2:policyName/scalingpolicytest
